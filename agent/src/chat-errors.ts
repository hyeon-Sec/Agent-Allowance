...
export function describeChatError(error: unknown, hasPaymentAttempt: boolean, provider: AIProvider = "anthropic"): string {
  const data = error as {
    status?: number; statusCode?: number; code?: string; type?: string; message?: string;
    error?: { code?: string; type?: string; message?: string; error?: { message?: string } };
  } | null;
  ...
  const type = data?.type ?? data?.error?.type;
  ...
  if (error instanceof ChatExecutionError) {
    ...
    reason = "Anthropic API 크레딧 잔액이 부족합니다. Anthropic 콘솔에서 API 크레딧을 충전해 주세요.";
      ...
      (type === "insufficient_quota" || code === "insufficient_quota" || code === "credit_balance_exhausted")) {
